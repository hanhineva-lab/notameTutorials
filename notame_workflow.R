# 1. Generate a peak abundance matrix from aligned metabolomics data
...
data <- import_from_excel(
  file = paste0(path, "toy_notame_set.xlsx"),
  sheet = 1,
  split_by = "Mode"
)
...
# 8. Perform the contaminant flagging, quality check, and drift
# correction on each mode separately within a loop and create
# visualisations of the various phases of the process in the figures
...
for (i in seq_along(modes)) {
  ...
  save_QC_plots(
    mode,
    prefix = paste0(path, "figures/", name, "_ORIG"),
    perplexity = 5,
    group = "Group",
    time = "Time",
    id = "Subject_ID",
  ...
  save_QC_plots(
    corrected,
    prefix = paste0(path, "figures/", name, "_DRIFT"),
    perplexity = 5,
    group = "Group",
    time = "Time",
    id = "Subject_ID",
  ...
  save_QC_plots(
    corrected,
    prefix = paste0(path, "figures/", name, "_CLEAN"),
    perplexity = 5,
    group = "Group",
    time = "Time",
    id = "Subject_ID",
...
save_QC_plots(
  merged,
  prefix = paste0(path, "figures/_FULL"),
  group = "Group",
  time = "Time",
  id = "Subject_ID",
  color = "Group"
...
save_QC_plots(
  merged_no_qc,
  prefix = paste0(path, "figures/FULL_NO_QC"),
  group = "Group",
  time = "Time",
  id = "Subject_ID",
  color = "Group"
...
# manually chosen representative ion columns (column Representative_ion,
# where the representative ions have been marked with x). Remove the extra
# rows, so that Feature_ID etc. become the column names (top rows). Save
# the Excel file under a new name, such as curated_data.xlsx

# 33. Join the manual annotation results from the Excel file (via
# openxlsx package) as extra data columns to the with_results object.
# Ensure that there is at least one redundant column common for both
# datasets, such as the unique Feature ID generated by notame
...
# 35. Calculate Spearman correlation coefficients and p-values between
...
# 36. Calculate Kendall’s τ correlation coefficients and p-values
# between the annotated metabolites and two sample information variables:
# time and injection order
...
# variance scaling, density curves, Group defining the colour, and Time
# defining the shape
plot_pca(
  object = imputed,
  pcs = c(1, 2),
...
# 39. Plot an arrow t-SNE including the first two principal
# components, unit variance scaling, density curves, the colour defining
# grouping variable 1, the time point for constructing the arrows, and
# subject ID. If the sample size is large and the image becomes crowded,
# it is possible to show the groups in separate faceted plots by adding +
# facet_wrap(~Group) at the end of the code
plot_tsne_arrows(
  object = imputed,
  scale = "uv",
  perplexity = 10,
...
# 40. Generate an effect heatmap from the correlation results between
# annotated metabolites and a set of other variables. Include only the
# manually annotated unique metabolites (see steps 17 and 18)
plot_effect_heatmap(
  correlations,
  x = "X",
  y = "Y",
...
# 41. Generate a volcano plot from a pairwise comparison by using the
# Cohen’s d and FDR-corrected p-values from the pairwise statistical test,
...
# 42. Save boxplots of all features by group, with “Curated ID” column
...
# 43. Save beeswarm plots of unique identified features (see steps 30
